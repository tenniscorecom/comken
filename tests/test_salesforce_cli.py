...
from comken.toolbox.salesforce.direct.cli import main
...
class TestPrintShape:
    def test_hides_secret_values(self, capsys):
        ...
        with patch("comken.toolbox.salesforce.direct.cli.Sandbox", return_value=client):
            main(
                [
                    "app",
                    "--domain",
...
class TestRotate:
    ...
    def test_aborts_when_not_confirmed(self, capsys):
        """確認に y 以外を入れたら何もしない。"""
        with (
            patch("comken.toolbox.salesforce.direct.cli.Sandbox") as sandbox,
...
class TestErrors:
    def test_returns_1_with_message(self, capsys):
        ...
        with patch(
            "comken.toolbox.salesforce.direct.cli.Sandbox",
