...
Python 側で組み立てる（`provider.report_folder()`）。**`担当者` / `概要` は
...
（`provider.report_folder()`）。**`担当者` / `概要` は記録用として管理表に残るが、
...
スケジュール行の「取得開始時刻」（スケジュール行が無いレポート＝後方互換は実行時点）。
**拡張子は常に `.csv`**。
...
**本日の「キャッシュ」（`cached_report()` / `cached_report_path()` が指す
...
| S004 | R001 | 1時間ごと | 09:00 |  |  |  | 取得する | ○ |
| S005 | R002 | 毎月 | 06:00 |  |  | 月末 | 取得しない | ○ |
| S006 | R003 | 毎月 |  |  |  | 第2営業日 | 取得しない | ○ |

**「取得開始時刻」列は「毎日」「毎週」「毎月」「1時間ごと」すべてに共通の実行
開始時刻**（この時刻を過ぎたら取得してよい）。「1時間ごと」は **取得開始時刻
から 60 分刻みで動く**（間隔の指定は無く、固定 60 分）。

**「取得開始時刻」列は空欄にできる**（`S006` がこの例）。

**「取得時刻」列はこのレポートが何時までに欲しいかの目安（記録用）。**
`is_due()` の判定ロジックには**組み込まない**ので、運用メモとして自由に使って
よい（例: 「17:00 までに欲しい」を書いておく）。空欄可。
...
（`取得頻度` / `祝日対応` / `有効` の 3 列。`曜日` / `日付` / `取得開始時刻` /
`取得時刻` は自由記述のため対象外。列の位置ではなく見出し名で探すので、列の
並び順は問わない）:
...
| ファイル名の付け方を変える | `provider.py` の `output_path()` ← **全プロジェクトに効く** |
