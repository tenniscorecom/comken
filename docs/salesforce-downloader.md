...
[Salesforce につないで確かめる](salesforce.md#つないで確かめるコマンド)。
...
雛形を作ってから場所を決めると、書き換えと移動を二度手間でやることになる。
...
| シート | 中身 |
|---|---|
| `管理表` | 見出し8列＋**記入例2行**。`実行方式`・`有効`・`0件あり` にドロップダウンが付く |
| `記入方法` | 各列に何を書くか、書けないときのエラー文 |

### 4. Excel で記入する

**記入例の2行は消す。** 薄い灰色の背景が付いていて、備考欄に
「記入例です。使う前にこの行を消してください」と書いてある。

書き方は「記入方法」シートを見る。要点だけ挙げると:
...
と例外の種類だけから機械的に決めている（[仕様書 4.28](開発/仕様書.md#428-レポート取得を集約し管理表と履歴で見えるようにする) 参照）。
