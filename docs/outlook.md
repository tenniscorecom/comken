...
> **BO 環境では利用できない。** BO には Classic Outlook がインストールされていないため、
> Outlook 関連モジュールを `import` した時点でエラーになる。BO 環境で動くツールを書く
> 場合は Outlook を import しないこと。intranet 環境では利用可能。
...
`received_at` は必ずタイムゾーンつきなので、`comken.now()` とそのまま比較できる。

> **添付ファイルの中身は取り出せない。** `has_attachments` で有無は分かるが、
> 保存するメソッドは用意していない。添付を取り込む必要が出たら、
> `comken.toolbox.outlook` に機能を足すことを検討する（各プロジェクトで COM を直接触らない）。
