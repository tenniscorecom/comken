...
4. [主要な設計判断](#4-主要な設計判断)
   ...
   - 4.34 デバッグモードは「開始/完了/中断」を出し、引数は出さず、config.ini から切り替える
...
| `core.wait` | core | 秒・分・条件成立までの待機 | 標準のみ |
| `core.transfer` | core | 転記共通処理（照合キー正規化、列名の検証） | exceptions |
| `core.files` | core | ファイル検索・操作・圧縮・命名方式 | exceptions・runtime・constants・core.clock |
| `core.files.naming` | core | 命名方式（1方式1ファイル） | core.clock |
...
- **config.ini に置くのは「非エンジニアが自分で変える値」だけ**。
  ...
  - **dry-run は config.ini に置く**（`[RUN] DRY_RUN`）。「本番前に1回試す」は
    非エンジニアが自分でやりたい操作で、そのためにコードを触らせたくない。
    ただし **True のまま戻し忘れると、毎日成功しているのに何も出力されない**状態になり、
    終了コードも 0 なのでスケジューラからは正常に見える。雛形の `main.py` は
    dry-run で動くたびに WARNING を出して気づけるようにしている。
    ライブラリ本体（`comken/runtime.py`）は `with dry_run():` のスイッチだけを
    持ち、`config.ini` を読むのは雛形側（`templates/新規プロジェクト/main.py`）
    の責務に切り出している。
  - **必須の項目は `config.require()` で起動時にまとめて確かめる**（4.21 と同じ考え方で、
    足りないまま動き出さない）。1件ずつ止めるのではなく足りないものを全部並べる——
    直して実行を繰り返す往復を減らすため
...
そこで照合キーの正規化と列名の検証を `core/transfer.py` に抜き出して、
両系統が同じ関数を経由するようにした。
`excel` から `windows` へ / `windows` から `excel` へ依存させないため、
共通部分は `core/`（どちらにも依存されない基盤）に置く。
...
から `python -m comken config` に変わった。
...
**`config.ini` の `[RUN] DEBUG` で切り替える。** dry-run と同じ形にした（4.1）。
止まったときに非エンジニアが自分で「デバッグモードで再実行」できないと意味がない。
dry-run と違って True のままでも業務は正常に流れるので WARNING は出さず、
INFO を1行出して「戻し忘れ」に気づきやすくする。`comken.require()` の対象にも足した
（�形 `main.py` で起動時にまとめて検証する形）。
