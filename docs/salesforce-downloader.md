...
**「取得頻度」列は「毎日」「毎週」「毎月」の 3 値から選ぶ。**

**「取得開始時刻」列は「毎日」「毎週」「毎月」すべてに共通の実行開始時刻**（この
...
が付いているので、ドロップダウンは自動で付く。既存の手入力（「月」「火」など
の漢字1文字、または「月曜日」のような接尾辞付き表記）も引き続き `weekday`
property でパースして受け付ける。

**「祝日対応」列は 4 値から選ぶ。** それぞれ次の意味:

| 値 | 意味 |
|---|---|
| `取得しない`（既定） | 対象日が祝日（`holidays` 引数に含まれていれば）なら、その日はスキップ |
| `取得する` | 対象日が祝日でも取得する |
