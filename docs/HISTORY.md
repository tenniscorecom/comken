...
判断に迷ったら **MINOR を上げて**旧名を残すと安全。ただし v2.0.0 では、この方針を
いったん外して破壊的変更をまとめて入れた（下の「v2.0.0」）。
...
138 クラスを 55 クラスにした（その後 Data Loader の削除で 54）。それ以外は、カテゴリ例外（`ExcelError` など）にメッセージをつけて直接送出する。
