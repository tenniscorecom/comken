"""comken/toolbox/salesforce/sites/developer.py — Developer 組織
...
class Developer(SalesforceBase):
    ...
    # My Domain の URL。Developer Edition は「<組織>--<Developer 名>.develop」の形になる。
    # 組織で固定なので config.ini には置かない（環境で変わる値だけを config.ini へ）
    # TODO: 配置するときに実際の URL へ置き換える
    DOMAIN_URL = "https://example--developer.develop.my.salesforce.com"

    # 認証情報のキー名の頭。DPAPI には developer_client_id / developer_client_secret で入る。
    # 別の登録に切り替えるときだけ Developer(prefix=...) で渡す
    CREDENTIAL_PREFIX = "developer"
