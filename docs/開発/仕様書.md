...
社内 RPA 基盤は社内 LAN だけに存在する。`comken/internal/rpa.py` /
`comken/internal/salesforce_api.py` で薄く包み、各プロジェクトはそこから
呼び出す。 **comken を差し替えるだけで全プロジェクトが追随する** という
運用上の利点はこのままで保つ。

社内ライブラリの import は **静的 import 文**（`from example_libs import rpa` など）を
`try / except ModuleNotFoundError` で囲み、 `comken.internal.base.raise_if_target_missing()`
...
- **静的 import にした理由**: pyright の型検査・IDE 補完が効き、初級エンジニアにも
  ...
  静的解析の効果が薄れる
- **`raise_if_target_missing()` ヘルパーに集約した理由**: 「対象自体（またはその親）が
...
- **「一元管理」（共通ルートから両名を組み立てる方式）は廃止した**: 
  ...
  `comken/internal/rpa.py` または `comken/internal/salesforce_api.py` のみ）

呼び出し側（`comken/internal/rpa.py` / `comken/internal/salesforce_api.py`）は、
関数の中で `from example_libs import rpa` を書いて `try / except ModuleNotFoundError` で
捕まえる形になる。関数の中で import するのは、社内ライブラリが無い PC でも
`comken` 自体を読み込めるようにするため（自宅・テストで import が通る）。
同じ静的 import を関数ごとに書くのは意図通りで、import に失敗しても `comken` 全体が
壊れないことを保証する。

**このリポジトリは公開しているため、社内ライブラリの実名は書かない。**
`example_libs.rpa` / `example_libs.salesforce` という仮の名前にしておき、
配置時に実際の名前へ書き換える（→ [docs/運用/配置.md](../運用/配置.md)）。
