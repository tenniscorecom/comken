...
**共有サーバーでは、この1ファイルを切り替えから守る**（配置時に1回だけ）。
...
`git checkout <タグ>`）。**社内固有の値を書いた1ファイル**（`comken/toolbox/salesforce/sites/sandbox.py`）は
