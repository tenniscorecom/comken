...
class Excel:
    ...
    def __init__(
        ...
        *,
        ...
        # 数式計算結果の読み取りは別クラスへ委譲する（``workbook.py`` を
        # 肥大化させないため、``ComputedValueReader`` が ``Excel`` を参照する）。
        self._computed = ComputedValueReader(self)
    ...
    def read(
        ...
        *,
        ...
        if self._engine == "com":
            ...
            return table
        rows = self._computed._read_computed_rows(sheet_name, header_row)
        if not rows:
            logger.debug(
