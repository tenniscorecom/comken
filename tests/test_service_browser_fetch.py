...
from comken.services.salesforce_downloader.service import _fetch, _fetch_via_browser
...
class TestFetchRoutesToBrowser:
    ...
    def test_uses_browser_when_key_is_listed(self, monkeypatch):
        monkeypatch.setattr(
            "comken.services.salesforce_downloader.service.BROWSER_FETCH_REPORT_KEYS",
            frozenset({ENTRY.key}),
        )
        table = MagicMock()
        with patch(
            "comken.services.salesforce_downloader.service._fetch_via_browser",
            return_value=table,
    ...
    def test_uses_api_when_key_is_not_listed(self, monkeypatch):
        ...
        with (
            patch("comken.services.salesforce_downloader.service.site_for") as site_for,
            patch(
                "comken.services.salesforce_downloader.service._fetch_via_browser"
            ) as fetch_via_browser,
        ):
...
class TestSeleniumStaysLazy:
    ...
    def test_fetch_does_not_import_browser_module(self, monkeypatch):
        ...
        with patch("comken.services.salesforce_downloader.service.site_for") as site_for:
