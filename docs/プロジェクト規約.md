...
**UTF-8・CRLF で保存する（BOM は付いていてもよい）。`chcp` は書かない。**

`chcp 65001` を書くと、cmd.exe が bat を読み進める位置がバイト単位でずれ、
**後続の `set "X=値"` や `set /p` が壊れる**（表示が崩れるのではなく、動かなくなる）。
コードページはコンソール側の設定に任せる。`tests/test_batch_files.py` が全 bat を検査する。

なお CP932 のコンソールで動かした場合は、日本語のメッセージが化ける（コマンドは壊れない）。
BOM を付けている場合は 1 行目がエラーになるため、そこへ配る予定があるなら BOM を外す。
