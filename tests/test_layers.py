...
ALLOWED_SAME_LAYER = {
    ...
    # toolbox.salesforce は api（本ファイル群）と browser（browser/ 配下）を同居させ、
    # 組織ごとの設定（URL・認証情報名）を1箇所に集約している。browser 側が Selenium を
    # 使う toolbox.browser に依存する（逆方向は無い。toolbox.browser は toolbox.salesforce
    # を知らない）
    ("toolbox.salesforce", "toolbox.browser"),
}
