...
**BOM なし UTF-8・CRLF で保存する。BOM と `chcp` は入れない。**
どちらも「表示が崩れる」ではなく「**動かなくなる**」side の失敗を起こす。

| やってはいけないこと | CP932 コンソールで起きること |
|---|---|
| BOM を付ける | BOM が 1 行目に食い込み `'@echo' は…認識されていません` |
| `chcp 65001` を書く | 読み進める位置がずれ、後続の `set "X=値"` や `set /p` が壊れる |

CP932 コンソールでは日本語のメッセージが化けるが、コマンド自体は壊れない。
表示の化けと動作不能なら、化ける方を選ぶ。`tests/test_batch_files.py` が全 bat を検査する。
