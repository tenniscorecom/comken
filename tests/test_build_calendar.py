...
class TestCabinetOfficeCsvLoader:
    ...
    def test_loads_real_format(self) -> None:
        ...
        build_calendar.SYUKUJITSU_CSV_PATH = FIXTURE_PATH  # type: ignore[misc]
        try:
            holidays = build_calendar._load_national_holidays()
        finally:
            build_calendar.SYUKUJITSU_CSV_PATH = original  # type: ignore[misc]
    ...
    def test_garbage_text_raises_value_error(self, tmp_path: Path) -> None:
        """内閣府 CSV として読めない（国民の祝日が 0 件）場合は ``ValueError``。"""
        original = build_calendar.SYUKUJITSU_CSV_PATH
        ...
        try:
            ...
            with pytest.raises(
                ValueError,
                match="内閣府 CSV から国民の祝日を 1 件も読み取れませんでした",
            ):
...
class TestCompanyHolidaysDefinition:
    ...
    def test_year_end_and_new_year_default_present(self) -> None:
        ...
        assert "年末年始休暇" in build_calendar.COMPANY_HOLIDAYS
        month_days = build_calendar.COMPANY_HOLIDAYS["年末年始休暇"]
    ...
    def test_extra_holidays_named_company_holiday(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(build_calendar, "COMPANY_HOLIDAYS_EXTRA", (_dt.date(2026, 11, 4),))
        rows = build_calendar.build_rows()
        # 国民の祝日と重なっていなければそのまま「会社休業日」
        assert (_dt.date(2026, 11, 4), build_calendar.EXTRA_HOLIDAY_NAME) in rows
        # 月日の会社休日ルールはそのまま
...
class TestBuildRows:
    ...
    def test_extra_holiday_does_not_overwrite_national(self) -> None:
        ...
        original = build_calendar.COMPANY_HOLIDAYS_EXTRA
        build_calendar.COMPANY_HOLIDAYS_EXTRA = (_dt.date(2026, 5, 4),)  # type: ignore[misc]
        try:
            rows = build_calendar.build_rows()
            ...
            build_calendar.COMPANY_HOLIDAYS_EXTRA = original  # type: ignore[misc]
    ...
    def test_is_independent_of_today(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        rows_a = build_calendar.build_rows()
        monkeypatch.setattr(clock_module, "today", lambda: _dt.date(2099, 1, 1))
        rows_b = build_calendar.build_rows()
...
class TestCsvSynchronization:
    ...
    def test_build_rows_matches_bundled_company_calendar_csv(self) -> None:
        ...
        assert bundled_header == ["date", "name"], (
            ...
            " `python tools/build_calendar.py` を実行して company_calendar.csv を"
            " 更新しコミットしてください。"
        )

        built = build_calendar.build_rows()
