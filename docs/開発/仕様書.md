...
| UC2 | CSV を Excel 帳票へ列転記 | `Transfer(CsvReader(...), writer.sheet(...), mapping).run()` |
...
利用者向け入口は ``Transfer(source, destination, mapping)`` に統一する。CSV は
``CsvReader`` / ``CsvWriter`` を直接渡し、Excel は既存 ``ExcelWriter`` から取得した
``Sheet`` を転記元・転記先の両方に使う。Transfer は指定した列を一方向へ転記する。mapping は必須で、
空なら全列を暗黙コピーする仕様にはしない。CSV → Excel、Excel → CSV、Excel → Excel、
CSV → CSV は同じ API で扱う。

行単位の公開APIは ``CsvReader.rows()`` と ``Sheet.rows()`` に揃え、どちらも列名付き辞書を
1件ずつ返す。Transfer内部もこのAPIだけを使い、形式別の転記クラスは作らない。

``run(*, transform=...)`` はキーワード限定の transform を必須とする。transform には転記元行と、
mapping の最初の転記元列に対応する値で検索した既存の転記先行を渡す。対応する転記先行がなければ
``None`` を渡し、同じキーに複数行が一致した場合は例外で停止する。どちらの辞書もコピーせず参照で
渡すため、transform は辞書を直接変更する。転記先行が ``None`` のまま操作されたために発生した
TypeError だけは、元の例外を ``__cause__`` に保持した Transfer 固有例外で補足する。その他の
TypeError は Python 本来の例外としてそのまま送出する。

通常処理では何も返さず、暗黙の ``None`` を継続として扱う。``Transfer.SKIP`` はその1件だけを
除外し、``Transfer.STOP`` は全体を打ち切る。それ以外の戻り値は処理規約違反として例外にする。
...
- **toolbox / services は公開の階層に上げない。** そこには「どの機能群に依存しているか」
  が読める意味があるので、`from comken.toolbox.excel import ExcelWriter` の
...
`git checkout <タグ>`）。**社内固有の値を書いた3ファイル**（`comken/toolbox/rpa.py`・
`comken/toolbox/salesforce/sites/sandbox.py`・`comken/services/salesforce_downloader/service.py`）は
