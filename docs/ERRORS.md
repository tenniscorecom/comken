...
| `SalesforceReportExportError` | 画面のエクスポート機能でレポートをCSV/XLS取得できなかった | 1. login_with_token() を呼んでからこのメソッドを呼んでいるか確認する2. 時間が経ってセッションが切れていないか（長時間のバッチの後半で発生する場合はこれが疑わしい）3. レポートそのものへのアクセス権・組織の Edition を確認してもらう |
