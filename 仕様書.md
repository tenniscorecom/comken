...
- **config.ini に置くのは「非エンジニアが自分で変える値」だけ**（2026-08-13 に明文化）。
...
社内のツールは RPA 基盤を通して動かすものが多いが、**基盤を通さず単体で動かすツールもある**。

- **2026-08-12 以前**: 「基盤を通さず単体で動かすツールは無い」という前提で、
  雛形は `backoffice(main, PROJECT_NAME)` に渡す形を既定にしていた
- **2026-08-13 に変更**: 単体実行するツールも作ることになったため、
  雛形は**単体実行を既定**にした。`setup_logging()` でログの出力先を用意してから
  `main()` を呼ぶ（基盤経由なら基盤がログを設定するので呼ばない）。
  RPA 基盤から動かす場合の書き方は、同じ `main.py` の末尾にコメントで示す。
  **雛形を2つに分けない**——`config.ini.example` や `docs/` が二重管理になり、
  片方だけ直す事故が起きるため
...
`Sheet.transfer_by_key()` は、ヘッダーがない帳票や列位置が仕様として固定された帳票向けに、
...
変換せず渡せるようにする。既存の `transfer_by_key()` は互換性を守るため、従来どおり
`{転記先の列記号: 転記元の列名}` とする。両者の向きは逆なので docstring で相互参照する。
