...
4. [主要な設計判断](#4-主要な設計判断)
   ...
   - 4.40 comken.toolbox.rpa を削除し comken.run へ移す（2026-09-07）
   - 4.41 「会社用カレンダーCSV 1本化」— 実行時は内閣府 CSV を知らない（2026-09-24）
5. [例外体系](#5-例外体系)
...
端末ごとの差分は `LOG_FOLDER_NAMES = {"PC-001": "bo-logs", ...}` のフォルダ名だけになる。
ログサーバーの移行（ルートだけ書き換え）と、端末ごとの登録変更（フォルダ名だけ）を
混ぜずに済ませるのが狙いで、移行のたびに全端末の `LOG_FOLDER_NAMES` 行を 1 行ずつ
書き換え直さずに済む。`setup_logging()` と `setup_local_logging()` はどちらが先でも動き、Console は
最初の呼び出しで得たものを 2 回目以降が使い回す（重複出力を避ける）。
未登録の端末は `LOG_ROOT/_etc/etc_{ホスト名}/` 配下に書かれ、業務は止めない。
未登録の端末が複数あってもログが混ざらないよう、ホスト名ごとのサブフォルダに分ける。
パス区切りを含む `LOG_FOLDER_NAMES` の値は `Path` の `/` 演算子で `LOG_ROOT` が
捨てられてしまう罠を避けるため未登録扱いとする。
...
社内 RPA 基盤は社内 LAN だけに存在する。`comken/toolbox/rpa.py` で薄く包み、
各プロジェクトはそこから呼び出す。 **comken を差し替えるだけで全プロジェクトが
追随する** という運用上の利点はこのままで保つ。
Salesforce API は comken 自前の `comken/toolbox/salesforce/` を使うため、
社内ライブラリ呼び出しの窓口は RPA のみになった。

社内ライブラリの import は **静的 import 文**（`from kensetsu_libs import rpa`）を
`try / except ModuleNotFoundError` で囲み、 `comken.toolbox.rpa._raise_if_target_missing()`
で `InternalLibraryNotFoundError` に変換する。動的 import（`importlib.import_module`
+ `importlib.util.find_spec`）は廃止した。バージョンを含まない仮名で確定したため、
動的 import の最大の理由（バージョン変動への対応）が消えた。

- **静的 import にした理由**: pyright の型検査・IDE 補完が効き、初級エンジニアにも
  素直に読める。動的 import は import 対象の存在を pyright が認識できないため、
  静的解析の効果が薄れる
- **`_raise_if_target_missing()` ヘルパーに集約した理由**: 「対象自体（またはその親）が
  見つからない」場合だけ `InternalLibraryNotFoundError` に変換し、モジュール内部の
  別の依存不足はそのまま呼び出し元へ伝える。 この判定は `ModuleNotFoundError.name`
  を直接見る必要があり、呼び出し側に書かせると事故りやすい
- **「一元管理」（共通ルートから両名を組み立てる方式）は廃止した**: 
  バージョンが固定されたので、共通の文字列を1か所に書く利点がなくなった。
  ライブラリ名自体が変わったら、各ファイルの定数を直接書き換える（影響は
  `comken/toolbox/rpa.py` のみ）

呼び出し側（`comken/toolbox/rpa.py`）は、関数の中で `from kensetsu_libs import rpa`
を書いて `try / except ModuleNotFoundError` で捕まえる形になる。関数の中で
import するのは、社内ライブラリが無い PC でも `comken` 自体を読み込めるようにする
ため（自宅・テストで import が通る）。 同じ静的 import を関数ごとに書くのは
意図通りで、import に失敗しても `comken` 全体が壊れないことを保証する。

**このリポジトリは公開しているため、社内ライブラリの実名は書かない。**
`kensetsu_libs.rpa` という仮の名前にしておき、配置時に実際の名前へ書き換える。
...
> **注記（2026-08-30 → 2026-08-31 に再統合）:** 2026-08-30 に
> `comken/services/salesforce_downloader` を `comken-salesforce-downloader`
> （別リポジトリ）へ一度分離したが、翌日中に再検討し、**comken 本体へ
> 再統合した。** 他プロジェクトから呼ぶには pip install が別途要ることに
> 気づいたため。`services` 層は現在 `salesforce_downloader` が入っており、
> 空ではない。詳細は 4.28・4.29 の注記を参照。
...
> **注記（2026-08-30 → 2026-08-31 に再統合）:** §4.27 で言及した `services` 層は、
> 2026-08-30 に一度空になったが、翌日中の再統合で `comken/services/salesforce_downloader`
> が戻り、現在は空ではない。層定義自体は 4 段のまま変えていない。
...
| 入口 | 数 | 基準 |
|---|---|---|
| `from comken import ...` | `len(comken.__all__)` | **何をするプロジェクトかに関係なく**使う土台 |
...
中身は設定・ログ・実行モードの3種類。**名前の一覧も具体的な数も、ここには書かない** —
使う側の一覧は README「使うときの約束」、機械可読な正本は `tests/test_facade.py`。
同じ列挙・同じ数値を複数の文書に置くと、どこか1箇所だけ古くなる（実際に comken 直下は
README が6個、仕様書が7個で食い違い、comken.core は仕様書が24個のまま何年分も
更新されず実数55個から乖離していた）。数を知りたいときは
`python -c "import comken; print(len(comken.__all__))"` のように実行して確認する。
**プロジェクト雛形が実際に import しているのは2つだけ**で、この範囲に収まっている。
...
> **注記（2026-08-30 → 2026-08-31 に再統合）:** 2026-08-30 に一度
> `comken-salesforce-downloader` （別リポジトリ）へ分離したが、翌日中に
> comken 本体へ再統合した。**以下の記述（レポート取得の集約という設計判断）は
> 現在も comken 本体でそのまま有効**（`comken/services/salesforce_downloader/`）。
...
- **タスク固有のレポート条件は `filters_by_report` で渡す。** キーはSalesforceの
  レポートIDではなく管理表の管理番号とし、管理表で参照先を差し替えても利用コードを
  変えずに済むようにする。条件を管理表へ追加しなかったのは、最終的に全レポートを
  SOQLへ移行する予定であり、移行時に同じ辞書を `WHERE` 句の材料として使えるよう
  コード上の構造化データとして残すため。未登録の管理番号は黙って無視せず
  `ReportNotRegisteredError` で止める
...
> **注記（2026-08-30 → 2026-08-31 に再統合）:** 2026-08-30 に一度
> `comken-salesforce-downloader` （別リポジトリ）へ分離したが、翌日中に
> comken 本体へ再統合した。**以下の記述（管理表を `report_master` で読み込む設計判断）は
> 現在も comken 本体でそのまま有効**（`comken/services/salesforce_downloader/report_master.py`）。
...
ジェネレータ関数（`Outlook.read_messages()` / `AccessDatabase.iter_rows()`）には
...
### 4.35 カレンダー判定は「国民の祝日 + 会社休日」を `comken.core.calendar` にまとめる

RPA 置き換えで「いま取るべきレポートか」を判定するために必要になった
カレンダー判定は、**内閣府の公開 CSV**（国民の祝日）と **会社の休業日**
を **マージ** して `comken.core.calendar` に置く。利用者が独自のカレンダー
を差し替える API は公開せず、**既定カレンダー 1 本だけ**を公開する。

国民の祝日は内閣府 CSV を内部実装の純粋計算ソース（mokejp/holidays_jp MIT 由来）の上に
マージしたものを遅延生成の `_Calendar` に保持する。**国民の祝日と会社休日が
同じ日に重なった場合は国民の祝日が先勝ち**（国民の祝日名称が返る）。

**内閣府の CSV は CP932（Shift_JIS）で配布され、列は
「国民の祝日・休日月日」「国民の祝日・休日名称」**。読み込みは
`comken.core.calendar.csv_source.load_cabinet_office_csv` に閉じ込め、
`CalendarFormatError` で止める。**内閣府以外を読み込もうとして
日付が 1 件も取れない**場合は止め、内閣府以外をカレンダー代わりに使う
事故を防ぐ。

**内閣府 CSV は `comken/core/calendar/data/syukujitsu.csv` に git 管理下で
同梱**しており、年 1 回の **手動更新**（開発機で内閣府から取得 →
コミット → 共有サーバーへ checkout）で配布する。**自動ダウンロード機能は
無い**（YAGNI: `comken.core` は `requests` 不要で動き、業務 BO 環境の
オフライン PC でも import できる）。

**期限切れは黙って判定せず、30 日未満で WARNING ログを 1 度だけ出す。**
`is_business_day()` で「収録最終日 <= 今日」を期限切れとみなし、
...
会社休日はルール判定なので期限を持たない（年末年始休暇は 2090 年でも休み）。

**会社休日（年末年始休暇など）は `comken/core/calendar/company.py` の
ルールで表現する**。**年が書いてない月日のリスト**で表現し、毎年の
メンテナンスが要らない（過去・未来を問わず同じ月日が休み）。
`COMPANY_HOLIDAYS`（年単位の月日）と `COMPANY_HOLIDAYS_EXTRA`
（特定年月日の例外）の 2 つの定数を `company.py` の管理者が編集する。
国民の祝日とは概念が違う（内閣府由来ではない）ので別モジュールに切り出し、
実行時の `company_holiday_name(date)` で毎回判定する。
...
- **何を**: rpa 系の後方互換シムと、改名ヘルパーを削除した
  ...
  - あわせて `comken/exceptions/__init__.py` の `__getattr__` シムと、
    当時の rpa 実装にあった未使用の `_load_rpa()`（「後方互換用」と
...
- **なぜ**: 8章の互換性ポリシーが定める「**全プロジェクトの grep で旧名の不使用を
  ...
  1 行だけで、そこを当時の rpa 呼び出しの新パスへ書き換えて削除に踏み切った。
...
文書（`docs/salesforce-downloader.md` / `docs/calendar.md` / `docs/csv.md` /
...
- **何を**: 「内閣府 CSV + 管理表の会社休日」の和集合をやめ、会社休日は
  `comken/core/calendar/company.py` の **コード直書き** だけにした
  （`COMPANY_HOLIDAYS` / `COMPANY_HOLIDAYS_EXTRA`）。内閣府 CSV は
  `comken/core/calendar/data/syukujitsu.csv` に**同梱**し、年 1 回手動更新する
...
> **追記（2026-09-03）**: `CabinetOfficeCSVSource` 自体を削除し、
> 内閣府 CSV は **年 1 回の手動更新**（開発機で取得 → コミット → 共有サーバーへ
> checkout）に一本化した。自動ダウンロード機能は完全に廃止。
...
- **いつ**: 2026-09-07
- **何を**: `comken/toolbox/rpa.py`（社内 RPA 基盤 `kensetsu_libs.rpa` への
  静的 import ラッパー）と、対応する例外 `comken/exceptions/rpa.py`
  （`InternalLibraryError` 系）を削除し、`comken/run.py`（`backoffice` /
  `intranet` の最小実装）に置き換えた
- **なぜ**: 社内側の RPA 基盤が再編され、外部パッケージ `kensetsu_libs.rpa` への
  依存が無くなった（内製化された）。もう存在しない呼び先を comken 側に
  残しておく意味が無いため、破壊的変更として削除した
- **中身は空実装**: `main()` を呼んで結果を返すだけ。実際のログ設定などの
  処理は社内固有のため comken には持たない（利用側で必要に応じて実装する）
- **付随する削除**: 外部ライブラリへの依存が無くなったことに伴い、社内
  ライブラリの実名が公開リポジトリへ漏れないようにする安全機構
  （`tools/export_for_chat.py` の `_verify_internal_library_placeholder()`）も、
  対象が無くなったため削除した
- **置き場所が変わった理由**: `comken.toolbox` は「〜を操作する／〜と通信する」
  部品の置き場所だが、外部ライブラリへの依存が無くなったことで
  `backoffice` / `intranet` は何とも通信しない単純な関数になったため、
  `comken` 直下（設定・ログ・実行モード・例外・定数と同じ層）へ移した
- **配置手順（`docs/運用/配置.md`）への影響**: `RPA_LIBRARY_NAME` を配置時に
  実名へ書き換える手順が不要になったため、該当節を削除した

### 4.41 「会社用カレンダーCSV 1本化」— 実行時は内閣府 CSV を知らない（2026-09-24）

- **いつ**: 2026-09-24
- **何を**: §4.35 の構成を大きく整理し直し、「国民の祝日＋会社休日を
  合成した 1 ファイル」を Python 実行時と VBA 側の両方が読む形にした

  - 新ファイル: `tools/build_calendar.py`（内閣府 CSV ＋ 会社休日ルール →
    `company_calendar.csv` を生成）
  - 新ファイル: `tools/calendar_data/syukujitsu.csv`（内閣府 CSV の置き場所を
    `comken/core/calendar/data/` から `tools/` 配下へ移した — 実行時の入力ではなく
    生成ツールの入力になったため）
  - 改名: `comken/core/calendar/data/holidays.csv` →
    `comken/core/calendar/data/company_calendar.csv`（Python・VBA 両方が参照する
    1 ファイル）
  - 削除: `comken/core/calendar/computed.py`（計算ソース）、`csv_source.py`
    （内閣府 CSV 解析）、`company.py`（会社休日ルール）
  - 実行時（`_calendar.py`）: 標準 `csv` だけを使って `company_calendar.csv` を
    読むだけのシンプルな実装に変更。`_ComputedSource` / `Holiday` /
    `approximate` / `company_holiday_name` / `_Calendar.from_csv` /
    `BUNDLED_CSV_PATH` / `EXPORTED_CSV_PATH` / `export_csv` を全て削除
...
- **公開 API の変更**:
  ...
  |---|---|
  | `BUNDLED_CSV_PATH` | 削除 |
  | `EXPORTED_CSV_PATH` | 削除 |
  | `export_csv()` | 削除 |
  | （同名・別物のパス） | `CALENDAR_CSV_PATH` |

- **例外階層の整理**:

  | 旧 | 新 |
  |---|---|
  | `CalendarSourceError` | 削除 |
  | `CalendarFormatError`（`CalendarSourceError` の子） | `CalendarError` の直接の子 |

- **なぜ**:
  1. **内閣府 CSV の形式変更の影響範囲を生成ツールだけに閉じる**。内閣府 CSV は
     配布形式が微妙に変わることがあり、その対応が面倒だった。実行時に内閣府 CSV
     の解析を持たなければ、対応コストは生成ツール（保守者）1 箇所に収まる
  2. **Python と VBA が同じ 1 ファイルを読む**。社内では VBA の既存資産が
     残っており、営業日判定を VBA から行う場合に同じ CSV をそのまま読めばよい。
     これまでは「Python は holidays.csv を生成して読む」「VBA は内閣府 CSV を
     直接読む」のように経路が 2 つあり、両者を毎年メンテする必要があった
  3. **コード量を大きく減らせる**。`_ComputedSource` は祝日計算の純粋計算で
     数百行あり、年単位の特例（2020 年オリンピック等）も保守負担だったが、
     生成ツールが内閣府 CSV をそのまま焼き込むため、計算ソースは完全に不要に
     なった
  4. **期限切れ検知の挙動がシンプルになる**。内閣府 CSV が「内閣府が公表して
     いる範囲（来年まで）」という制約を持っていたため、これまでは
     `_ComputedSource`（1948-2099 まで計算）で「内閣府 CSV の範囲外」を
     カバーしていたが、生成ツールが内閣府 CSV だけを採用するため、
     範囲外は `False` 倒し（誤って「祝日扱い」より誤って「平日扱い」の方が
     安全なため）。会社休日は会社用カレンダーCSV の収録範囲にしか入らないが、
     同じ判断を統一
- **互換性ポリシーとの関係**: `BUNDLED_CSV_PATH` / `EXPORTED_CSV_PATH` /
  `export_csv()` / `CalendarSourceError` を削除した破壊的変更。利用側の
  参照がないことを `grep` で確認済み（利用側 0 件）。共有サーバーへ配置する
  前のタイミングでの破壊的変更のため、互換性ポリシーの旧名残置ルールは
  適用しない
- **再生成**: 既存の `syukujitsu.csv`（1955-2027）をそのまま使い、
  `python tools/build_calendar.py` で `company_calendar.csv` を 1 ファイル
  だけ再生成した。**生成物の中身は内閣府 CSV と会社休日ルールだけで決まり、
  呼ぶ日に依存しない**（同期テストで担保）

### 4.42 配布方式を「git チェックアウト」から「フォルダコピー（robocopy）」へ変更（2026-09-24）

- **いつ**: 2026-09-24
- **何を**: 配布方式を以下の通り変更した
  - 新規: `tools/build_release.py`（git のある開発 PC で動く。タグ時点の
    ファイルを `git archive` で取り出し、`dist/comken-<タグ>/` と zip、
    `RELEASE.txt`、robocopy コマンドを生成）
  - 廃止: 共有サーバーでの `git fetch --tags && git checkout <タグ>` 手順
  - 廃止: 共有サーバー側での `git update-index --skip-worktree` による
    3ファイル保護（BO/intranet の各ネットワークに git バイナリが無い）
  - 置き換え: robocopy の `/XF` で3ファイルを送り元（配布用フォルダ）側のフルパス指定で除外
- **なぜ**: BO と intranet の共有サーバーには git バイナリが入っていない
  （ユーザー確認済み、2026-09-24）。これまで共有サーバー上で git を
  チェックアウトしていた手順は、両環境で動かないことが分かった。
  `git archive` の出力をそのままコピーする方式に変えた。`git
  update-index --skip-worktree` の保護は、共有サーバー側に git が無い
  ため使えず、代わりに robocopy の `/XF` で送り元（配布用フォルダ）側のフルパスを
  指定して除外する形にした
- **互換性**: 配置手順の変更のみで、公開 API やタグの運用ルールは変えていない
- **影響範囲**: 配布手順のオペレーション（手順書の実行者向け）。`build_release.py`
  自体の実行は git のある開発 PC で行うため、ツールの依存は増えていない
...
| `comken/toolbox/salesforce/sites/solution_sandbox.py` | サンドボックス組織の My Domain・認証情報の接頭辞 |
| `comken/services/salesforce_downloader/paths.py` | レポート管理表・履歴を置く共有フォルダ |

> **注記（2026-08-30 → 2026-08-31 に再統合 → 2026-09 に取得実行部分のみ再分離）:**
> Salesforce レポートの集約取得は 2026-08-30 に `comken-salesforce-downloader`
> （別リポジトリ）へ分離されたが、**2026-08-31** に comken 本体へ再統合された。
> その後 **2026-09** に、取得を実行する側（`download_scheduled()` / 旧 `service.py`）
> だけが単一消費者（`Salesforceレポートダウンローダー`）向けに再度切り出され、
> 管理表・履歴の形式（このファイルを含む）は comken 側に残った。`_paths.py` は
> この再分離を機に、外部プロジェクトから import できる名前として `paths.py`
> （アンダースコア無し）へ改名した。配置時に書き換える対象であることは変わらない。

**設定ファイルへ集約しない。値は使う場所に書く。** 一度 `settings.ini` へ、次に
`settings.py` へ集約してみたが、どちらも良くなかった。

- **ini にすると、設定がコードと ini に散る。** どちらを見ればよいか分からなくなる
- **1つの Python ファイルに集めても、書き換えるファイルは減らない。** それに加えて、
  `Solution` の URL が `Solution` クラスから離れる。
  **クラスの値はそのクラスにあるほうが、探さずに済む**

**書き換え忘れは必ずエラーになる**（組織へつながらない・管理表が見つからない）ので、
仮名のまま黙って動いて間違った結果を出すことはない。

**共有サーバー側では、この3ファイルを配置のたびに上書きしない。** 配布ツール
（`tools/build_release.py`）が robocopy の `/XF` で送り元（配布用フォルダ）側のフルパスを
除外する。除外した3ファイルは各サーバー側の値のまま残り、共有一方の
サーバー（BO・intranet）で値が違う場合も壊れない。
...
使われるので、コードには置けない。共有サーバーのリポジトリが変わったら、
プロジェクト側の bat と `.vscode/settings.json` を書き換える必要がある
（旧 `setup_comken.bat` による恒久登録は廃止 — 下記注記を参照）。
...
```
共有サーバー \\server\share\tools\comken   ← build_release.py の出力（タグ時点のフォルダ）
...
**開発と本番の分離:** 共有サーバーには**`tools\build_release.py` の出力だけを置く**
（2026-09-24 決定）。開発中のものを置かず、配布する直前のリリース用フォルダだけを置く。
配布ツールがタグ時点のファイルだけを取り出すため、配置＝リリースの版固定がそのまま
成立する。**BO と intranet の共有サーバーは同じタグ・同じ番号を使う**（2026-09-24 決定）。
差は「Outlook は BO では使えない」という注記だけで、コードは分けない。

> **注記（2026-09-08）:** リポジトリ直下の `setup_comken.bat`（PC ユーザー環境変数へ
> `PYTHONPATH` を恒久登録するスクリプト）は廃止。会社側の PC イメージ配布で
> `PYTHONPATH` が設定済みのため、利用者側で個別セットアップを実行する必要が
> 無くなったため。共有フォルダの移動時は、プロジェクト側の `実行.bat` /
> `認証情報の登録.bat` / `.vscode/settings.json` の3か所を手で書き換える。

更新は **開発 PC で `tools\build_release.py` を実行**してリリース用フォルダを作り、
各環境の共有サーバーへ `robocopy /MIR` でコピーする（コマンドはツールが表示する）。
...
robocopy の `/XF` で送り元（配布用フォルダ）側のフルパスを指定して除外し、各サーバー側の値が上書き
されないようにする。

以前は git のチェックアウトで切り替える方式だったが、**2026-09-24 に「git の無い
共有サーバーへフォルダコピー」へ変更した**。BO と intranet の両ネットワークには
git が入っていないため、git チェックアウトは使えない。配布ツールは開発 PC で動く
（git が必要）。配布ツール自体は共有サーバーにコピーしない（**共有サーバーには
git がない前提で動く**ので、ツールの追加は配置作業を複雑にするだけ）。
...
`RELEASE.txt` を見ないと分からなくなる。

### 開発とリリース

**開発は手元のクローン、共有サーバーは `tools\build_release.py` の出力だけを指す。**
共有サーバーを更新した瞬間に、次に import した全プロジェクトへ伝播するため、
「いつ切り替わるか」を人が決められる形にする。

```
手元のクローン（F:\dev\comken）              共有サーバー（\\server\share\tools\comken）
    master に開発をコミット                              build_release.py の出力（タグ時点のフォルダ）
        ↓ リリースするとき                              ↓ 配布すると決めたときだけ
    __version__ を上げ、タグを打って push    ──→    robocopy /MIR（BO/intranet 両方）
```

**master に直接コミットしてよい。** 1人で開発しているので、develop ブランチを分けても
「どちらが最新か」を管理する手間が増えるだけになる。**共有サーバーが build_release.py の
出力を指しているので、master に何をコミットしても本番には流れない。**
...
| 4 | コミットしてタグを打つ | `git commit` → `git tag v1.0.0` → `git push && git push --tags` |
| 5 | リリース用フォルダを作る | `python tools\build_release.py --tag v1.0.0`（タグと `__version__` の整合性を検査し、`dist/comken-v1.0.0/` と `dist/comken-v1.0.0.zip` を作る。robocopy コマンドも表示） |
| 6 | 共有サーバーへコピー | 表示された robocopy コマンドを **BO 用と intranet 用の両方の共有サーバー**で実行する（利用中でない時間帯に） |

**6 をやるまで本番は変わらない。** 1〜5 は好きなときにやってよい。
...
**2026-09-24 に v1.0.0 をリリースした。** 祝日カレンダーの整理（`comken.core.holidays` → `comken.core.calendar`）など、破壊的変更をまとめて入れた最初の安定版で、**以降は上の SemVer ルールと互換性ポリシーに従う**（破壊的変更は原則やらない。やるときは旧名を残す移行期間を設ける）。バージョンの定義は `comken/__init__.py` の `__version__` の1か所だけ。リリースのたびにここを上げ、同じ番号のタグを打つ。

**BO と intranet でバージョンを分けることはしない**（2026-09-24 決定）。どの版が動いているかが分からなくなるのは、版を固定しているために起きる問題で、版を分けても解決しないため。環境ごとの差は、同じタグを使ったうえで、各環境が使える機能・依存の範囲（`docs/` の各モジュールの説明）で扱う。
...
旧リリースのフォルダ/zip を使って同じ手順でコピーし直す。
`dist/` は git 管理外（`.gitignore` で除外）なので、**配布した zip は手元に残しておく**
（あるいは必要なら `python tools\build_release.py --tag <旧タグ>` で古いタグから
同じフォルダ/zip を再生成できる）。

```bat
:: 古いタグのフォルダを古いタグで再生成する場合
python tools\build_release.py --tag v0.7.0

:: 表示された robocopy コマンドを BO/intranet 両方の共有サーバーで実行する
```

#### 共有サーバーで気をつけること

- **`tools\build_release.py` の出力だけを置く。** 開発中のものを置かない（直した
  はずが反映されていない、という事故を防ぐため）
- **社内固有の3ファイルは robocopy の `/XF` で除外する。** `build_release.py` の
  robocopy コマンドに最初から含まれているので、運用では何も追加しなくてよい。
  手動で robocopy を組み立てる場合も同じ3ファイルを `/XF` に並べる
- **共有サーバーには git がない前提で運用する。** 配置・切り替えに git コマンドを
  使う手順は使えない（BO/intranet の各ネットワークには git バイナリが無い）。
  `build_release.py` は開発 PC で動かすもので、共有サーバーへコピーしない
- **`/MIR` は宛先にしか無いファイルを削除する。** 配布先に開発中に置いたファイルが
  あっても、配布と同時に消える。除外指定した3ファイルは削除されない
