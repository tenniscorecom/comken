...
| `toolbox.rpa` | 社内 RPA 基盤の処理を遅延 import して呼び出す | 社内ライブラリ・exceptions |
| `settings` | 社内固有の値（共有フォルダの場所・組織の URL・レポート ID） | 標準のみ |
| `services.salesforce_downloader` | 管理表（Excel）に沿ったレポート取得と履歴の記録 | excel・csv・salesforce・settings |
