...
- **何を**: 配布方式を以下の通り変更した
  - 新規: `tools/build_release.py`（git のある開発 PC で動く。タグ時点の
    ファイルを `git archive` で取り出し、`dist/comken-<タグ>/`（`RELEASE.txt` 付き）と
    robocopy コマンドを生成。zip は作らない）
...
| 5 | リリース用フォルダを作る | `python tools\build_release.py --tag v1.0.0`（タグと `__version__` の整合性を検査し、`dist/comken-v1.0.0/` を作る。robocopy コマンドも表示） |
...
旧リリースのフォルダを使って同じ手順でコピーし直す。
`dist/` は git 管理外（`.gitignore` で除外）なので、**配った `dist\comken-<旧タグ>\` フォルダを保管しておく**
（無ければ `python tools\build_release.py --tag <旧タグ>` で古いタグから
同じフォルダを作り直せる）。
