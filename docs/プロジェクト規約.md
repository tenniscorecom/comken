...
my_project/
  ...
  src/
    run.py ← 処理の本体
    credentials.py ← 使う認証情報の宣言（REQUIRED_CREDENTIALS）
...
REQUIRED_CREDENTIALS = {
    "SALESFORCE": ["client_id", "client_secret"],  # キーは config.ini [CREDENTIALS] のキー名
    "OJU_SYS": ["password"],
}
```

登録は非エンジニアが GUI で行える。`templates/認証情報の登録.bat` をプロジェクトのルートに
コピーしておくと、**ダブルクリックで登録画面（`python -m comken.credentials --gui`）が開く**。
そのプロジェクトの `REQUIRED_CREDENTIALS` のうち未登録の項目が一覧表示される。
