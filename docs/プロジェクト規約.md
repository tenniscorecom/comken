...
comken 直下から import してよいのは、プロジェクト全体の土台となる `config` / `Config` と
実行モード4関数だけ。それ以外は `from comken.excel import ExcelWriter` のように、
機能を提供するパッケージを明示する。ワイルドカード import は禁止する。
詳細と良い例・悪い例は [CONVENTIONS.md「import の書き方」](../CONVENTIONS.md#import-の書き方) を参照。

ログの設定（出力先・フォーマット・レベル）は社内の共通ライブラリ側で行う。
利用プロジェクトでは `logging.getLogger(__name__)` を使うだけでよい。
