...
``calendar`` を省略できる。省略時は **既定カレンダー**（``ComputedHolidaySource`` +
同梱 ``syukujitsu.csv`` + ``CompanyHolidaySource``）が使われ、
ネットワークには出ない。会社独自の休業日はこのライブラリの管理者が
``comken/core/holidays/sources/company.py`` の ``COMPANY_HOLIDAYS`` /
``COMPANY_HOLIDAYS_EXTRA`` にコードで追加する
（呼び出し側から足すものではない）。

``set_default_calendar()`` / ``HolidayCalendar.from_sources()`` も
公開しているが、通常は使わない（テストや特殊用途向け）。
