...
配置するときに書き換えるのは次の2つだけ。

| ファイル | 何を |
|---|---|
| `comken/settings.py` | 共有フォルダの場所・組織の My Domain・認証情報の接頭辞・レポート ID |
| `comken/toolbox/rpa.py` | 社内 RPA 基盤の import 行（2か所） |

**設定は ini に分けず、Python の定数にまとめる。** 設定がコードと ini に散ると、
どちらを見ればよいか分からなくなる。**探す場所は1つにする。** Python の定数なら
import した時点で解決するので、クラス定数（組織の `DOMAIN_URL`）にもそのまま使える
（ini から読む形だと、import した瞬間に設定ファイルが必要になり、用意していない環境では
import 自体が落ちる）。補完も型も効き、書き間違いは import した時点で分かる。

`rpa.py` だけ別なのは **import 文は文字列にできない**ため。動的 import にすれば
`settings.py` へ寄せられるが、「保守する人が Python に詳しいとは限らないので、動的 import の
間接さより素直さを取る」という 4.19 の判断と矛盾する。社内ライブラリの正式版が出れば
消えるファイルでもある。
...
`settings.py` に置くのは「comken を1回配置したら、そのまま変わらない値」だけにする。

各プロジェクト側の comken の場所（`実行.bat` 等の4か所）は**Python が起動する前**に
使われるので `settings.py` には置けない。こちらは `set_comken_root.py` でまとめて
