...
def _read_nested(json_path: Path) -> dict[str, dict[str, str]]:
    ...
    if not parsed:
        ...
        raise _import_error(json_path, "取り込む項目が1つもありません。")
...
def _validate_and_collect_site(json_path: Path, site: str, raw_fields: object) -> dict[str, str]:
    ...
    if not site:
        raise _import_error(json_path, "サイト名が空です。")
    ...
    if not isinstance(raw_fields, dict):
        raise _import_error(json_path, f"「{site}」の中が {{ }} で囲まれていません。")
    fields: dict[str, str] = {}
...
def _validate_field(json_path: Path, site: str, field: str, value: object) -> None:
    """サイト配下の項目名と、 その値を検証する。"""
    if not field:
        raise _import_error(json_path, f"「{site}」の中に項目名が空のエントリがあります。")
    ...
    if not value:
        raise _import_error(json_path, f"「{site}」の「{field}」が空です。")
...
def _read_json_object(json_path: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise _import_error(json_path, f"ファイルを読めませんでした（{e}）。") from e
    try:
        ...
        raise _import_error(json_path, f"「{e.key}」が2回書かれています。") from e

    if not isinstance(parsed, dict):
        logger.debug("_read_json_object: 最上位がオブジェクトでない: json_path=%s", json_path)
        raise _import_error(json_path, "いちばん外側が { } になっていません。")
...
# 取り込み時の形式エラーは、すべて ``CredentialError`` を直接送出して具体的な
# 状況をメッセージで伝える。


def _import_error(path: Path, detail: str) -> CredentialError:
    """取り込む JSON が壊れている・形式が違うときの ``CredentialError``。"""
