...
（`python -m comken stubs` 等）案がある**。既存の `credentials` /
...
| `from comken.core import ...` | 23 | 特定の操作対象（ファイル・日時・文字列・差分・計測）を持つ部品 |
...
[docs/ライブラリ開発規約.md](ライブラリ開発規約.md#サイト組織クラスを昇格させる基準) を案内する。
...
（→ [docs/ライブラリ開発規約.md](ライブラリ開発規約.md#管理者が扱うのは連絡が来たものだけ)）。
...
現在のWindowsユーザーの`PYTHONPATH`と`PATH`へリポジトリルートを追加する（`PATH` に入るので
`comken init` と打てる）。既存値と**値の型（`REG_EXPAND_SZ` / `REG_SZ`）を保ったまま**書き、
