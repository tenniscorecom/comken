...
直下の7個は `config` / `Config`（設定）、`setup_logging`（ログ）、
`dry_run` / `is_dry_run` / `debug` / `is_debug`（実行モード）。
**プロジェクト雛形（`templates/新規プロジェクト/`）が実際に import しているのは
`setup_logging` と `config` の2つだけ**で、この範囲に収まっている。
