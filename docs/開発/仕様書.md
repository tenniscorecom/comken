...
**共有サーバーでは、この2ファイルを切り替えから守る**（配置時に1回だけ）。
...
`git checkout <タグ>`）。**社内固有の値を書いた2ファイル**（`comken/toolbox/salesforce/sites/solution.py`・
`solution_sandbox.py`）は `git update-index --skip-worktree` で保護しておく（配置時に1回だけ）。
