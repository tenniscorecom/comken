...
共有サーバーへ配置するときに、次の3か所を実際の値へ書き換える。

| ファイル | 何を | 仮の値 |
|---|---|---|
| `comken/toolbox/rpa.py` | 社内 RPA 基盤の import 行（2か所） | `example_libs.v0000.rpa` |
| `comken/toolbox/salesforce/sites/sandbox.py` | 組織の My Domain・レポート ID | `example--sandbox...` / `00O000000000001` |
| `comken/services/salesforce_downloader/service.py` | 管理表と履歴の置き場所 | `\\server\share\tools\salesforce\...` |

- `MASTER_PATH` は**人が編集する Excel**（レポート管理表）、`HISTORY_PATH` は
  **プログラムが追記する CSV**（ダウンロード履歴）。comken 本体とは別のフォルダに置く
- 社内ライブラリのバージョンが上がったときも `rpa.py` の同じ2行を直す

**書き換え忘れは必ずエラーになる**（社内ライブラリが見つからない・組織へつながらない・
管理表が見つからない）ので、仮名のまま黙って動いて間違った結果を出すことはない。

各プロジェクト側の comken の場所（`実行.bat` 等の4か所）は `set_comken_root.py` で
まとめて書き換えられる（→ README「comken の場所を変えたとき」）。
