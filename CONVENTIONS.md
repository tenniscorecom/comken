...
`comken/exceptions/` には既に 110 個以上の例外クラスがある。**新しいクラスを
際限なく足すのは避ける**ため、次の基準を両方満たすことを確認してから追加する。

| 基準 | 意味 |
|---|---|
| **利用者側の対処が既存の例外と違う** | 「同じ例外でも別メッセージにしたいだけ」では新しいクラスを足さない。既存クラスにメッセージを足すか、送出側で文脈を組み立てる |
| **`docs/ERRORS.md` に「意味」と「自分でできる対処」を書ける** | 書けないなら、それは利用者に独立した失敗として見せる必要がない＝既存の例外で済ませる場面 |

次のいずれかに当てはまるときは **足さない**:

| 状況 | 採るべき対応 |
|---|---|
| 同じ対処で済む別パターン（例: パスの形式違い・サイズ違い・権限違いで別クラス） | 既存の例外にメッセージを足す、または `__init__` で文脈を込めて送出する |
| 「基底クラス（`ComkenError` の直下）」を増やしたい | **増やさない。** `ExcelError` / `CSVError` / `SalesforceError` 等の既存分類に置く。基底を増やすと `except ComkenError` でまとめたい側にとって分類が細かくなりすぎる |
| 既存例外の docstring に「対処:」を書き足せば足りる | 例外クラスは足さず、 docstring を拡張する |
| 1 プロジェクトでしか出ない固有の手順由来のエラー | ライブラリに置かず、 プロジェクト側で `ProjectError` のようなローカル基底を定義する。共有フォルダ配布のライブラリに固有手順が入ると他プロジェクトの障害調査ノイズになる |

**「足してよい」例:**

- `ConfigMappingEmptyValueError` のように、 **対処が他の Config 例外と違う**
  （「対応表を直す」＝非エンジニアの操作が必要で、`ConfigKeyNotFoundError` の
  「セクション名を直す」とは別作業）。
- 既存クラス名だと `except` で取り分けできないが、利用者の業務フロー上は
  **別のリカバリ手順** に繋ぎたいケース。
...
**統合するときの基準**: 対処が同じで、利用側が型で区別する必要がないものだけを 1 クラスにまとめ、
違いはメッセージで示す（例: `ComkenFileNotFoundError` は何のファイルかをメッセージに出す）。
統合の前に、利用側プロジェクトを grep して、**送出・捕捉・型による分岐・クラス名の記録**に使われていないかを確かめる
（comken 内で送出されていないだけでは「未使用」と判断しない。利用者プロジェクトが送出する想定の例外がある）。
統合したら旧名は削除せず、`comken/exceptions/__init__.py` の `_RENAMED_EXCEPTIONS` に登録して
`FutureWarning` つきの別名として残す。
