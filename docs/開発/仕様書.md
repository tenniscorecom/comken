...
社内ライブラリの import は **静的 import 文**（`from example_libs import rpa`）を
...
呼び出し側（`comken/toolbox/rpa.py`）は、関数の中で `from example_libs import rpa`
...
`example_libs.rpa` という仮の名前にしておき、配置時に実際の名前へ書き換える
