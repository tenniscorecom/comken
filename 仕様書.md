...
4. [主要な設計判断](#4-主要な設計判断)
   ...
   - 4.8 公開 API は最小限
   - 4.9 ログ設定は持たない
   - 4.10 日時は PC のローカルタイムゾーンで扱う
   - 4.11 Access は CSV 直接出力を主役にする
   - 4.12 Access は既定でローカルコピーを開く
   - 4.13 元 DB を更新する場合は、ローカルコピーを書き戻さない
   - 4.14 元 DB を直接開く前に日時付きバックアップを取る
   - 4.15 Outlook は Classic のみ対応し、送信しない
   - 4.16 補助関数より本体が受け入れる
5. [例外体系](#5-例外体系)
...
| UC2 | CSV を Excel 帳票へキー突合転記 | `CsvReader.index` → `Sheet.transfer_by_key` → `save`（数式再計算・PW付き保存が必要なら `ExcelComHandler` 版） |
...
- 数式を書くのは `ExcelWriter` で足りる。COM が必要なのは計算結果を Python 側で使うときと、
  計算済みの状態でPDF化・印刷・他システムへ渡すとき
- VBA マクロ・パスワード付き保存が必要なときも `ExcelComHandler`（COM。Excel 必須）
- `ExcelWriter` は必要時に内部で COM へ自動フォールバックする
- openpyxl は数式を検証しないため、数式を書いた重要なブックは
  `ExcelComHandler.recalculate()` で再計算・検査する。既定では、書き間違いを強く示す
  `#NAME?` と参照先の消失を示す `#REF!` を異常とする。`#N/A` や `#DIV/0!` は
  業務上正常なことがあるため、既定では異常としない
- 検算は `written_ranges()` を渡し、自分が書いた範囲に絞るのが既定の使い方。
  長く使われている業務ブックには無関係な古いエラーが残っていることがあり、全体検査では
  自分の書き込みが正しくても止まるため。テーブル名の変更、シートの削除、行や列の削除など
  参照を壊す変更をしたときだけ `ranges` を省略して全体を検査する
