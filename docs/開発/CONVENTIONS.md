...
| 規約 | 対象 | 内容 |
|---|---|---|
| **本書（CONVENTIONS.md）** | すべての Python コード | 命名・型・定数・例外・ロギング・コメント・テスト等の言語レベルの共通ルール（詳細版） |
| [docs/開発/コーディング規約_利用者向け.md](コーディング規約_利用者向け.md) | comken を**使う側**のプロジェクト | 本書から利用者に必要な最小限だけを抜き出した薄い版。配布・共有はこちらを使う |
| [docs/開発/ライブラリ開発規約.md](ライブラリ開発規約.md) | **comken 本体**を編集する人 | 公開 API（`__all__`）・カスタム例外の階層設計・互換シム・パッケージ構成 |

comken を使う側のプロジェクトの構成は `templates/新規プロジェクト/` の雛形が、
使い方は [README](../../README.md) と各モジュール文書が持つ。
...
- **同じことをする書き方を2つ作らない。** 別名・薄い委譲メソッド・「便利のため」の二重の入口を置かない。覚える量が増え、どちらが正なのかを読む人に判断させることになる。1つに決め、他は削除する。役割が本当に違うもの（例: `save()` は途中で確定する、`with` 終了は最後に確定する）だけは残し、docstring に違いを書く。
...
新しい語を持ち込む前に、動詞表の語（`read_` / `find_` / `fetch_` など）で
要件を満たせないか検討する。

- 良い例: SOQL の `query()`。「読む」ではなく「クエリ言語で問い合わせる」
  という動詞表に無い操作なので、`read_` にしない判断は正しい。
- 悪い例: レコードを 1 件取得するメソッドを `fetch_record()` にした。
  動詞表の「1 件を検索する」＝ `find_` や CRUD の慣習的な `get()` と同じ操作なので、
  「業界でよく使われる語だから」という理由だけで新しい語を持ち込まない。
...
**「ファイルがどこにあるか」と「中身をどう読むか」は分ける。** パス（フォルダ・
ファイル名）は `[FILES]`、シート名・見出し行のような読み取り設定は `[EXCEL]`
（CSV なら `[CSV]`）に置く。`[FILES]` と `[EXCEL]` のキー名は揃えておくと、
どの設定がどのファイルに対応するか目で追える。
...
[EXCEL]
; [FILES] の INPUT_受注_EXCEL に対応する読み取り設定
