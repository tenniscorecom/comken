"""``comken.services.salesforce_downloader.sheets.group_settings`` の動作を検証する。
...
class TestProviderCache:
    """``provider._load_group_settings_cached()`` のキャッシュ挙動。"""
    ...
    def test_caches_by_resolved_path(self, tmp_path):
        ...
        try:
            ...
            second = provider_module._load_group_settings_cached(tmp_path / "設定.xlsx")
            assert first == second
            # provider のキャッシュ経由でも同じ dict が返る
            third = provider_module._load_group_settings_cached(tmp_path / "設定.xlsx")
            assert first == third
        finally:
            provider_module._reset_cached_master()
