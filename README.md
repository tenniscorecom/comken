...
など、どこに置いても動く**。両 bat は comken の場所を次の順で自動判定し、最初に見つかった
ものを採用する（判定はどの候補でも `<候補>\comken\__init__.py` の存在で行う）。

| 優先 | `setup_comken.bat` | `comken.bat` |
|---|---|---|
| 1 | 第1引数で明示したパス | （引数は `init` に使うため省略） |
| 2 | bat 自身のフォルダ（`%~dp0`） | bat 自身のフォルダ（`%~dp0`） |
| 3 | 現在の `PYTHONPATH` の各エントリ | 現在の `PYTHONPATH` の各エントリ |

リポジトリの外（たとえば `setup_comken.bat` をデスクトップにコピー）から走らせるときは、
引数で comken の場所を渡す:

```bat
.\setup_comken.bat \\server\share\tools\comken
```

`comken.bat` は引数を `init` に使うので、置き場所は PYTHONPATH から自動で辿る
（先に `setup_comken.bat` を一度走らせて `PYTHONPATH` を登録しておくのが前提）。

### セットアップ手順（新しい PC ではじめにやること）

1. リポジトリ直下の `setup_comken.bat` を実行する
   → このフォルダが現在の Windows ユーザーの `PYTHONPATH` と `PATH` に追加される。
   既に同じパスが登録済みなら重複追加しない。bat 自身の場所（または引数）から comken の
   場所を判定するので、編集は不要
...
git checkout v0.11.0       :: 切り替えたいタグ（上で確認した最新版）
popd
```
