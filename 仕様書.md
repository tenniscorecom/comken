...
BO用の別配置先へ反映するときは`deploy_comken.bat`を使う。バージョン更新を必須にし、Ruffと
pytestに成功したコードだけを一時フォルダ経由で切り替える。既存版は`backup/`へ退避し、
配置情報は`DEPLOYMENT.txt`へ残す。これにより、手作業コピーによるバージョン更新忘れと、
コピー途中のコードを実行側が読む事故を防ぐ。
