...
| UC2 | CSV を Excel 帳票へ列転記 | `Transfer(CsvReader(...), ExcelWriter(...), mapping, destination_sheet=...).run()` |
...
利用者向け入口は ``Transfer(source, destination, mapping)`` に統一する。CSV / Excel は
ともに列名付き辞書を返し、Transfer は指定した列を一方向へ転記する。mapping は必須で、
...
シート全体の値・書式の複製は列マッピングとは責務が異なるため、``Sheet.copy_to()`` に残す。
