...
**「出力ファイル名」「保存方式」は必須列。** 空欄や `○` `×` `上書き` `新規` 以外の値で
書くと、`MasterRowValueError` で読み込みが停止する（書き忘れを即座に検知するため）。
`enabled` 列と同じ考え方。
...
| ID | 概要 | Salesforce URL | グループ | 担当者 | 有効 | 0件あり | 2000件超 | SOQL | 出力ファイル名 | 保存方式 |
|---|---|---|---|---|---|---|---|---|---|---|
| 1001 | 顧客一覧 | https://.../Report/00O5g00000ABCDE/view | 営業本部 | 山田太郎 | ○ | × | × | × | 月次受注.csv | 上書き |
| 1002 | 売上実績 | https://.../Report/00O5g00000FGHIJ/view | 営業本部 | 佐藤花子 | ○ | ○ | × | × | 売上実績.csv | 新規 |
...
| **出力ファイル名** | 既存の社内RPA（9291）が読みに行く固定ファイル名。拡張子込みで書く（例: `月次受注.csv`）。拡張子を省略した場合は自動的に `.csv` を補う。**必須列**（空欄はエラー） |
| **保存方式** | `上書き` か `新規`。「上書き」は出力ファイル名の固定名で毎回上書きする（9291互換）。「新規」は取得のたびに別名（スケジュール時刻 `YYYYMMDD_HHMM` を付けた名前）で残す。**必須列**（空欄や上記2値以外はエラー） |
...
<ベースパス>\<担当者>\<概要>\1001_顧客一覧_20260814.csv          ← 当日キャッシュ
<ベースパス>\<担当者>\<概要>\月次受注.csv                        ← 9291 向け
```
...
### 既存の社内RPA（9291）向けの追加出力

全レポートについて、既存の履歴ファイル・当日キャッシュとは**別に**、社内RPA
（9291）が読みに行くための固定名/準固定名ファイルが追加で出力される
（「出力ファイル名」「保存方式」は必須列のため、対象外にする選択肢は無い）。
組み立て方は `provider.rpa_output_path()`
（`comken/services/salesforce_downloader/provider.py`）が担当する。

- **`保存方式 = 上書き`**: 「出力ファイル名」をそのまま保存先に使う（拡張子が
  省略されていれば `.csv` を補う）。毎回同じパスへ上書きされる想定で、9291 は
  このパスを見に行く
- **`保存方式 = 新規`**: 「出力ファイル名」の stem に、根拠のスケジュール行の
  `取得時刻` を `YYYYMMDD_HHMM` の形で付けた名前を保存先に使う。スケジュール行が
  無いレポート（後方互換）のときは現在時刻で代用する。衝突した場合は `_1` / `_2`
  と連番を付けて既存ファイルを残す（既存の `_reserve_path()` と同じアルゴリズム）

「出力ファイル名」「保存方式」の**両方が必須列**。空欄や `上書き`/`新規` 以外の
値を入れると `MasterRowValueError` で `load_master()` ごと停止する（書き忘れ検知）。
...
| 9291（既存RPA）向けの固定名ファイル出力 | 管理表の「出力ファイル名」「保存方式」列（コードは触らない） |
