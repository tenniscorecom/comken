...
4. [主要な設計判断](#4-主要な設計判断)
   ...
   - 4.3 ExcelFile と ExcelComHandler の使い分け
...
| `const` | `Encoding` / `Color` / `FileFormat` / `SortBy` の公開定数 | なし |
| `naming` | ファイル名などの命名方式（現在は `DateNameBuilder`） | 標準のみ |
| `utils` | ファイル検索・ダウンロード管理・データ変換 | 標準のみ |
...
`FileFinder.today/latest` 等は見つからない場合に即例外
...
- config / csv / excel（openpyxl）/ utils / logger / exceptions / pdf / runtime をカバー
