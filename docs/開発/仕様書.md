...
> **v1.0.0で方針変更。** 利用者向け入口は ``Transfer(source, destination, mapping)``
> に統一する。CSV / Excel はともに ``rows()`` で列名付き辞書を返し、Transfer は
> 一方向の列マッピングだけを担う。シート全体の複製は ``Sheet.copy_to()`` として分離する。
> 既存の列位置版は、固定帳票を扱う内部部品としてのみ残す。
