...
| `handoff` | 取得側と利用側の間で、日付入りファイルの置き場所と不足を管理 | exceptions・utils.clock |
...
| UC7 | 複数プロジェクトで同じ取得結果を使う | 取得 → `Handoff.path_of` へ保存 → `Handoff.require` → 利用・配布 |
...
### 4.27 取得と利用を受け渡しフォルダで分ける

同じデータを複数プロジェクトで使う場合は、取得を1本にまとめ、結果を受け渡しフォルダへ
置く。利用側は取得手段を知らず、必要なファイルがあるかどうかだけを条件にする。これにより、
取得が失敗しても人が同じ名前で置けば、利用側を変更せずに続きから動かせる。

日付はフォルダではなくファイル名に入れ、前日分の誤使用を防ぐ。増減するレポートと配り先は
config.ini やコードへ列挙せず、1行1レポートの管理表で持つ。詳しい運用と理由は
[受け渡し](docs/handoff.md)を正本とする。
