...
def _run_doctor(_args: argparse.Namespace, remaining: list[str]) -> int:
    """``python -m comken doctor`` の本体。

    環境の切り分け診断（依存・設定・接続）をまとめて行う。
    ``--json`` で機械可読な JSON を出せる（CI 等から利用）。
    """
