"""examples/advanced/soql_report_migration/run.py — SOQLレポート移行の一連の流れを実演する。
...
（comken 自身のテスト ``tests/test_soql_reports.py`` と同じ手法）。
**``_fake_site()`` とそれを差し替えている ``with patch(...)`` の節は、このサンプル
だけの都合。** 本番コードでは一切書かない — ``site_for()`` が ``LargeSalesReport.URL``
から実際の組織を自動解決する。

実行方法:
    リポジトリのルートで python -m examples.advanced.soql_report_migration.run
...
from .large_sales_report import OUTPUT_DIR, LargeSalesReport
...
def main() -> None:
    ...
    with patch(
        "comken.services.salesforce_downloader.soql_reports.runner.site_for",
