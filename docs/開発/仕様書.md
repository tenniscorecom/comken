...
| `services.salesforce_downloader.report_master` | services | 管理表（Excel）のスキーマ DSL（`MasterRow` / `column`） | toolbox.excel・exceptions |
...
社内環境では `setup(Backoffice)` のように環境クラスを渡して root logger を設定する。
二重出力を見逃さないため、root logger が設定済みなら例外にする。単体実行では `local()`
...
書き換え直さずに済む。`setup()` と `local()` はどちらが先でも動き、Console は
最初の呼び出しで得たものを 2 回目以降が使い回す（重複出力を避ける）。
未登録の端末は `LOG_ROOT/_etc_/` 配下に書かれ、業務は止めない。
...
`setup()` / `local()` の処理自体は続行する。名前を「**誤って**共存させてしまう」のではなく
「**意図的に**共存させる」ことを示すため、キーワード引数で書かせる設計にした。
comken の handler が両方走った後にもう一度 `setup()` / `local()` を呼ぶケースは
...
これに加え、``unmatched_read_rows()`` で転記先に無い read 行を、
``unmatched_write_rows()`` で転記元に無い write 行を、それぞれ独立に取り出せる。
新規行の追加は ``unmatched_read_rows()`` の戻り値を ``transfer.result().append()``
に渡す形で行う（mapping で write の全列を埋める自動追加はしない。mapping に
無い列の初期値は利用者だけが知っているため）。利用者は ``for`` / ``if`` /
``continue`` で普通の Python として ``write_row`` を加工する。同じキーに
複数の転記先行がある場合は曖昧な更新をせず例外で停止する。加工の結果は ``_working_table``
（Transfer 内部の作業用 ``Table``）に反映され、入力の ``read`` / ``write`` は変わらない。

**空キー (``None`` / ``""``) は突合対象外**。 空キーは read / write のどちらでも
照合に使わず、対応が無ければ ``unmatched_read_rows()`` /
``unmatched_write_rows()`` 側へ流れる。 ``0`` / ``False`` は空ではない
...
> （`examples/excel_key_transfer` で実証済み）。
...
**辞書で読まない。** `read_computed_rows_as_dicts()` でも読めるが、使う側が毎回 `row["名前"]` と
...
**管理表は `services.salesforce_downloader.report_master` と同じ「人が Excel で編集する表」路線を採る**。
`ComkenMasterTableSource` は Excel の「会社休日」シートから「日付」「名称」を
読んで `Holiday` に変換する。シート名・列名は既定を持たせつつ、
プロジェクト側の事情に合わせて上書きできる（既定は `services.salesforce_downloader.report_master` と同じく
日本語見出し。「日付」「名称」）。
...
| `comken/services/salesforce_downloader/_paths.py` | 管理表と履歴の置き場所 |
...
`comken/toolbox/salesforce/sites/sandbox.py`・`comken/services/salesforce_downloader/_paths.py`）は
