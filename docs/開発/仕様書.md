...
| 参照方法 | 共有サーバーに置き、各プロジェクトの `認証情報の登録.bat` が実行中だけ `PYTHONPATH` を設定して直接参照（8章参照。`pip install` はしない） |
...
各プロジェクト側の comken の場所（`認証情報の登録.bat` 等の2か所）は**Python が起動する前**に
