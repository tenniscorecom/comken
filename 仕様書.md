...
### 4.17 社内ライブラリの呼び出しは comken が吸収する
...
`comken/rpa.py` で包めば **comken を差し替えるだけで全プロジェクトが追随する**。

**このリポジトリは公開しているため、社内ライブラリの実名は書かない。**
`LIB_ROOT` / `LIB_VERSION` を仮の名前にしておき、配置時に実際の値へ書き換える。
実名を書き戻さないこと。

ログの出力先など「毎回書かされる決まり文句」は `_prepare()` に集める。
呼び出し側を変えずに、全プロジェクトへ一度に効かせられる。

### 4.18 config.ini が無ければ example から作り、そこで止める
...
### 4.19 config.ini のセクション名・キー名は大文字を強制する
