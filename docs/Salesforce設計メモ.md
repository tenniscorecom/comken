...
| 取るもの | 用途 |
|---|---|
| 呼び出し回数（組織別・エンドポイント別） | 使用量の把握 |
| 呼び出し元コンポーネント | どこが API を食っているか。クラスに1回持たせて分類する |
| リトライ回数（401 再認証 / 5xx / 制限超過を区別） | 不安定さの検知 |
| 所要時間 | `comken.utils` の `Timer` を流用 |
...
これは撤去前の `credentials.dat`（DPAPI で暗号化した JSON）と同じ形式なので、
**保存側はそのまま流用し、入口だけ差し替える**。
...
- 撤去前は対話式 CLI で 1 件ずつ登録していた。今回は**JSON を食わせる**形にする
- 組織ごとに client_id / client_secret が別なので、`site_a` / `site_b` のプレフィックスで分ける
