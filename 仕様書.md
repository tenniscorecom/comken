...
| `deprecation` | 公開 API の旧名を使ったときに移行先を警告する | 標準のみ |
| `credentials` | Windows DPAPI による認証情報の暗号化保存・読み込み・JSON 取り込み | pywin32・exceptions・utils.files |
...
| `access` | Access マクロ・VBA・保存済みクエリ実行、テーブル／クエリの CSV 出力・逐次読み取り | pywin32・Microsoft Access |
...
| `salesforce` | Salesforce API の認証・データ操作・レポート実行・認証情報ローテーション | requests・credentials・exceptions・runtime・utils.clock |
| `run` | 社内 RPA 基盤の処理を遅延 import して呼び出す | 社内ライブラリ・exceptions |
| `logger` | 日付別ファイルとコンソールへのログ設定 | runtime・utils.clock |
| `state` | state.ini への処理状態の保存・読み込み | exceptions・runtime・utils.files |
...
`docs/`が二重管理になるため、1つにまとめる。

プロジェクト名（RPA 基盤へ渡す名前）は `new_project.py` が作成時に差し込む。
...
| 場所 | 基準 |
|---|---|
| 直下 | 何を操作するかに関係なく使う（config / runtime / logger / state / exceptions / constants） |
...
共有サーバーへ配置するときに、実際の値を入れる場所は3つ。

| 何を | どこに書くか | 仮の値 |
|---|---|---|
| 管理表と履歴の置き場所 | **`settings.ini`**（git 管理外） | `\\server\share\tools\salesforce\...` |
| 社内 RPA 基盤の import 行（2か所） | `comken/toolbox/rpa.py` を書き換え | `example_libs.v0000.rpa` |
| 組織の My Domain・レポート ID | `comken/toolbox/salesforce/sites/sandbox.py` を書き換え | `example--sandbox...` |

**コードの書き換えは `git pull` で衝突する。** 共有サーバーの comken は git チェックアウト
なので、値をコードに書くと更新のたびに手作業になる（コミットすれば公開リポジトリに社内の
実名が載る）。だから**設定にできるものは `settings.ini` に置く**（git 管理外なので
pull で消えない。無ければ example から作って、その時点で止まる）。

設定にできないものが2つある。

- **`rpa.py` は import 文**なので設定から読めない。動的 import にすれば可能だが、
  「保守する人が Python に詳しいとは限らないので、動的 import の間接さより素直さを取る」
  という 4.19 の判断と矛盾する。社内ライブラリの正式版が出れば消えるファイルでもある
- **`sandbox.py` はクラス定数**なので、設定から読むと **import した瞬間に settings.ini が
  必要**になり、用意していない環境では `import` 自体が落ちる。避けるには基底クラスの
  設計変更が要るため、別途扱う

**書き換え忘れは必ずエラーになる**（社内ライブラリが見つからない・組織へつながらない・
settings.ini が無い）ので、仮名のまま黙って動いて間違った結果を出すことはない。

各プロジェクト側の comken の場所（`実行.bat` 等の4か所）は**Python が起動する前**に
使われるので settings.ini には置けない。こちらは `set_comken_root.py` でまとめて
書き換える（→ README「comken の場所を変えたとき」）。
...
各PCで恒久的に参照する場合は、リポジトリ直下の`install_pythonpath.bat`を1回実行し、
現在のWindowsユーザーの`PYTHONPATH`へリポジトリルートを追加する。既存値を保持し、重複は
追加しない。共有フォルダの移動時は古い値をユーザー環境変数から削除して再実行する。

BO用の別配置先へ反映するときは`deploy_comken.bat`（中身は`deploy.py`）を使う。Ruffと
pytestに成功したコードだけを、配置先の中に作った一時フォルダ経由で切り替える。既存版は
`backup/`へ退避し、配置情報は`DEPLOYMENT.txt`へ残す。コピー途中のコードを実行側が読む事故と、
手作業コピーによるバージョン更新忘れ（配置先と同じバージョンなら止める）を防ぐ。
...
| 2 | テストと lint を回す | `python -m pytest tests -q` と `python -m ruff check .` が両方きれいなこと |
| 3 | main にマージ | 開発はブランチ自由。main = リリース可能な状態、を維持する |
| 4 | 共有サーバーのチェックアウトを更新 | 共有サーバー上で main を `git pull`（または リリースタグを `git checkout`）。`release-YYYYMMDD-HHMM` のタグを打っておくとロールバックしやすい |
