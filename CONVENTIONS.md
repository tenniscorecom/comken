...
| 種別 | 例 | 備考 |
|---|---|---|
| 一般的な略語（クラス名） | `CSVError`, `APIMetrics`, `ExcelCOMHandler`, `InvalidReportURLError`, `CalendarError` | すべて大文字 |
...
`comken/exceptions/` には既に 120 個以上の例外クラスがある。**新しいクラスを
...
**既存の例外の統廃合はしない。** 影響が大きいため。本節は「これ以上増やすのを
止める基準」を明文化するだけで、過去のものは触らない。
