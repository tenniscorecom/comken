...
| `services.salesforce_downloader` | 管理表（Excel）に沿ったレポート取得と履歴の記録 | excel・csv・salesforce |
...
使われるので、コードには置けない。こちらは `set_comken_root.py` でまとめて書き換える
（→ README「comken の場所を変えたとき」）。恒久登録（`install_pythonpath.bat`）を
