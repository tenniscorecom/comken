...
|---|---|---|
| UC1 | NAS 上の当日 Excel を加工して出力 | `FileFinder.today` → `Excel`（大容量は自動ローカルコピー）→ 加工 → `save` |
| UC2 | CSV を Excel 帳票へ列転記 | `Transfer(CSV(...), writer.sheet(...), mapping).run()` |
...
- 読み取りだけなら `Excel`、書き込みを伴う場合は `Excel`（openpyxl。Excel 不要・高速）
...
`Excel` は読み取り専用、`Excel` は読み書き用とする。読み取り処理を対等な別部品にすると、
...
二重出力を見逃さないため、root logger が設定済みなら例外にする。単体実行では `local()` が
返す名前付き logger を使い、コンソールとローカルファイルのレベルを個別に指定できる。
...
既定にし、`local()`でログを設定してから`main()`を呼ぶ。RPA基盤から動かす場合の
...
`Transfer.run(transform)` または `write_table()` で書き出す。
...
行単位の公開読み取りはコピーを返す。Transfer 内部だけが、コピーした作業 Table の実体行を
明示的に更新する。形式別の転記クラスは作らない。

``run(*, transform=...)`` の transform は任意で、指定する場合はキーワード引数で渡す。Transfer は
``read_key`` と ``write_key``（複合キーも可）で行を照合し、最初に mapping を作業行へ適用してから、
転記元行のコピーと作業行のコピーを transform へ渡す。対応する転記先行がなければ transform は
呼ばず、その行を飛ばす。同じキーに複数の転記先行がある場合は曖昧な更新をせず例外で停止する。
transform が作業行を変更しても、入力の read / write は変わらない。

transform を省略した場合は mapping だけを適用する。指定した場合、通常処理では何も返さず、
暗黙の ``None`` または ``True`` / ``Transfer.APPLY`` を適用として扱う。``False`` /
``Transfer.SKIP`` はその1件の変更を破棄し、``Transfer.STOP`` は全体を打ち切る。それ以外の
戻り値は、転記元の行番号を含む ``TransferRowError`` とする。
既存の転記先データはキー検索と更新に利用し、全件削除は行わない。Python の iterator はループ
本体が ``continue`` したか通常完了したかを識別できないため、Transfer 自身を iterable にはしない。
...
- **core** — 直下にだけ依存する部品。外にあるものは触らない
  （logger / state / config / clock / text / data / transfer / retry / timer / wait / files）
...
- **toolbox / services は公開の階層に上げない。** そこには「どの機能群に依存しているか」
...
`salesforce` / `salesforce_downloader` の `__main__` と並びを揃える話なので、
後で3つの CLI をまとめて扱うときに検討する。
...
| `from comken.services.xxx import ...` | 各パッケージの `__all__` | 社内の決まりに沿った仕組み |
...
`comken.toolbox.holidays.csv_source.load_cabinet_office_csv` に閉じ込め、
...
（`is_business_day` / `next_business_day` / `expires_after`）は requests 不要で動く**
（社内 BO 環境で `requests` が入っていない PC でも import できる）。
`CabinetOfficeCsvSource.load()` の冒頭でキャッシュの TTL を確認し、
TTL 内なら取得を呼ばない（ネットワークを無駄に叩く）。
TTL 経過時は内閣府から取得し、**取得に失敗してもキャッシュが残っていれば
警告ログのみで動く**（オフラインでも 1 度落としてあれば動き続ける）。

**期限切れは黙って判定せず、30 日未満で WARNING ログを 1 度だけ出す。**
`expires_after()` で「収録最終日 <= 今日」を期限切れとみなし、
「切れた瞬間まで気付かない」を避けるため 30 日前段階で `HolidayCalendarExpiringWarning`
相当の警告を出す。同じ日に 2 回 `is_business_day` を呼んでも 1 度しか出さない
...
**管理表は `toolbox.master_table` と同じ「人が Excel で編集する表」路線を採る**。
`ComkenMasterTableSource` は Excel の「会社休日」シートから「日付」「名称」を
読んで `Holiday` に変換する。シート名・列名は既定を持たせつつ、
プロジェクト側の事情に合わせて上書きできる（既定は `toolbox.master_table` と同じく
日本語見出し。「日付」「名称」）。
...
| `comken/internal/salesforce_downloader/_paths.py` | 管理表と履歴の置き場所 |
...
使われるので、コードには置けない。こちらは `tools/set_python_library.py` でまとめて書き換える
...
`git checkout <タグ>`）。**社内固有の値を書いた3ファイル**（`comken/internal/rpa.py`・
`comken/toolbox/salesforce/sites/sandbox.py`・`comken/internal/salesforce_downloader/_paths.py`）は
