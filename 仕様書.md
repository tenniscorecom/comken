...
元 DB を直接開く場合は、開く前に日時付きのバックアップを実行フォルダ直下の `backup/` へ取り、
既定で7日間残す。非エンジニアが実行フォルダを開けば自分で見つけられる場所に置くためである。
プロジェクトが共有フォルダ上にある場合は、ネットワーク越しの書き込みが遅いうえ、
破損リスクを避ける目的も損なうため、`backup_dir` でローカルフォルダを指定する。
