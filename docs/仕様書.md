...
手元のクローン（F:\dev\original_libs）          共有サーバー（\\server\share\tools\comken）
    ...
    __version__ を上げ、タグを打って push    ──→    git fetch && git checkout v0.10.0
```
...
| 4 | コミットしてタグを打つ | `git commit` → `git tag v0.10.0` → `git push && git push --tags` |
| 5 | 共有サーバーを切り替える | 共有サーバーで `git fetch --tags` → `git checkout v0.10.0` |
