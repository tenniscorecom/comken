...
``transfer_rows()`` で転記元の全行（転記先に無ければ ``write_row=None``）を返す。利用者は
``for`` / ``if`` / ``continue`` で普通の Python として ``write_row`` を加工する。同じキーに
...
``write_row[write_col] = read_row[read_col]`` を実行して適用する。Python の iterator は
