...
| `comken/services/salesforce_downloader/_paths.py` | レポート管理表・履歴を置く共有フォルダ |

> **注記（2026-08-30 → 2026-08-31 に再統合）:** Salesforce レポートの集約取得は
> 2026-08-30 に `comken-salesforce-downloader` （別リポジトリ）へ分離されたが、
> **2026-08-31** に comken 本体へ再統合された。`_paths.py` は分離・再統合の間も
> 一貫して配置時に書き換える対象。
...
`solution_sandbox.py`・`comken/services/salesforce_downloader/_paths.py`）は
